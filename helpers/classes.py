...
from helper import valueCheckErr, clear
...
class Literator:
    ...
    def selection(self, question):
        print(f"{question} Pierwszemu (1) czy drugiemu (2)")
        try:
    ...
    def readWords(self):
        clear()
        self.words[0] = (str(input("Podaj pierwsze słowo: ")))
        self.words[1] = (str(input("Podaj drugie słowo: ")))
    ...
    def whichLonger(self):
        if len(self.words[0]) > len(self.words[1]):
            ...
            return(f"Oby dwa słowa ({self.words[0]} i {self.words[1]}) są tej samej długości ({self.words[1]} znaków)")

    def capitalize(self):
        if self.selection(self,"Któremu słowu chcesz zwiększyć literę?"):
            self.capitalize(self)

        self.words[self.selectedWord] = self.words[self.selectedWord].upper()

    def lower(self):
        if self.selection(self,"Któremu słowu chcesz zmiejszyć literę?"):
            self.lower(self)
    ...
    def whichHeavier(self):
        ...
        if word1Weight > word2Weight:
            return(f"Pierwsze słowo ({self.words[0]}) \"ważące\" {word1Weight} jest cięższe od drugiego słowa ({self.words[1]}) ważącego {word2Weight}")
        elif word2Weight > word1Weight:
            return(f"Drugie słowo ({self.words[1]}) \"ważące\" {word2Weight} jest cięższe od pierwszego słowa ({self.words[0]}) ważącego {word1Weight}")
        else:
